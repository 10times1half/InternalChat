...
.DESCRIPTION
    ...
    자가진단: powershell.exe -File .\InternalChat.ps1 -SelfTest
.NOTES
...
# P7: 백그라운드 HTTP용 순수 .NET 헬퍼 (스레드에서 PS 스크립트 실행 안 함)
try {
...
# ###########################################################################
# 인라인 라이브러리 (Logger/Security/Data/Api/UI)
# ###########################################################################
...
function Save-AppConfig {
    <#
    .SYNOPSIS
        data/config.json 저장 (서버/폴링/알림/창위치 등).
    #>
    param(
        [string]$ApiBase,
...
function Import-ApiBaseFromConfig {
    ...
    # 폴링 간격 고정 (설정 무시)
    $script:PollIntervalMs = 15000
    if ($script:PollTimer) { $script:PollTimer.Interval = 15000 }
...
#region P1 AppState (메모리 인덱스 + dirty flush)

function Initialize-AppState {
    <#
    .SYNOPSIS
        P1: 인메모리 인덱스/dirty 초기화.
    #>
    $script:UserById = @{}
    $script:ConvByMd5 = @{}
...
function Import-AppStateFromDisk {
    <#
    .SYNOPSIS
        users / conversations / sync 디스크 로드. dirty=false.
    #>
    Initialize-AppState
...
function Save-SyncState {
    <#
    .SYNOPSIS
        호환 래퍼: Set-SyncState (dirty). 디스크는 Save-AppStateDirty.
    #>
    param(
        [Parameter(Mandatory = $true)][string]$LastSync,
        [Parameter(Mandatory = $true)][string]$LastMessageId,
...
function New-ConversationRecord {
    <#
    .SYNOPSIS
        대화 메타 PSCustomObject (미확인 개수 unreadCount 포함).
    #>
    param(
        [string]$Md5,
        [string[]]$ParticipantIds = @(),
...
function Update-ConversationMeta {
    <#
    .SYNOPSIS
        대화 메타 메모리 upsert. DirtyConversations=true.
        IncrementUnread 시 unreadCount 증가, ClearUnread 시 0.
    #>
    param(
        [Parameter(Mandatory = $true)]
        [string]$Md5,
...
function Get-RecentChatMessages {
    <#
    .SYNOPSIS
        최신 월부터 읽어 최근 $Take 건만 반환 (전체 월 full load 회피, P2).
    .OUTPUTS
        시간 오름차순 메시지 배열 (최대 Take)
    #>
    param(
        [Parameter(Mandatory = $true)]
        [string]$Md5,
    ...
    # months: 최신순. 월 단위 청크를 최신부터 모은 뒤 시간순 flat + 꼬리 N
...
function Update-ConversationListView {
    <#
    .SYNOPSIS
        conversations 로 ListView 갱신.
        P2: 순서가 같으면 행 내용만 갱신, 다르면 전체 재구성.
    #>
    param(
        [Parameter(Mandatory = $true)]
        [System.Windows.Forms.ListView]$ListView,
...
function Initialize-UserListView {
    ...
    # 행 클릭/선택 시 체크 (여러 명 선택 시 각각 체크 유지)
...
function Format-MessageDateTime {
    <#
    .SYNOPSIS
        표시용 일시 YYYY-MM-DD HH:mm:ss (24시간)
    #>
    param($Value)
    if (-not $Value) { return '---- -- -- --:--:--' }
    try {
...
function Add-ChatMessageToView {
    <#
    .SYNOPSIS
        A안 RTF 스타일 메시지 표시 (버블 느낌).
        이름/시간 회색 소형, 본문 강조, 나=우측, 상대=좌측.
    ...
    $title = ''
    if ($Message.ad) { $title = [string]$Message.ad.Trim() }
    # 제목이 본문 앞과 같으면 생략 (쪽지 subject=본문 앞 10자 패턴)
    if ($title) {
        $plainOne = ($body -replace '\s+', ' ').Trim()
        if ($plainOne.StartsWith($title) -or $title -eq $plainOne) {
            $title = ''
        }
    }
    ...
    $fontBase = $RichTextBox.Font
    if (-not $fontBase) {
        $fontBase = New-Object System.Drawing.Font('맑은 고딕', 9.5)
    }
    $fontMeta = New-Object System.Drawing.Font($fontBase.FontFamily, [Math]::Max(8.0, $fontBase.Size - 1.0), [System.Drawing.FontStyle]::Regular)
    $fontBody = New-Object System.Drawing.Font($fontBase.FontFamily, $fontBase.Size, [System.Drawing.FontStyle]::Regular)
    $fontTitle = New-Object System.Drawing.Font($fontBase.FontFamily, $fontBase.Size, [System.Drawing.FontStyle]::Bold)

    $colorMeta = [System.Drawing.Color]::FromArgb(120, 130, 140)
    $colorBodyOwn = [System.Drawing.Color]::FromArgb(20, 90, 160)
    $colorBodyOther = [System.Drawing.Color]::FromArgb(30, 30, 30)
    $colorTitle = [System.Drawing.Color]::FromArgb(60, 70, 80)
    $colorBody = if ($isOwn) { $colorBodyOwn } else { $colorBodyOther }

    $nl = [Environment]::NewLine

    # 메타: 이름 · YYYY-MM-DD HH:mm:ss
    $meta = '{0}  ·  {1}' -f $namePart, $timeStr

    $append = {
        param($txt, $font, $color)
    ...
    & $append ($meta + $nl) $fontMeta $colorMeta

    if ($title) {
        & $append ($title + $nl) $fontTitle $colorTitle
    }

    & $append ($body + $nl) $fontBody $colorBody
    ...
    $RichTextBox.SelectionFont = $fontBase
...
$script:PollAsyncRunning    = $false  # P4 백그라운드 폴링 중
...
$script:BalloonWhenChatHidden = $true  # 고정: 항상 알림
$script:BalloonOtherChat     = $true  # 고정: 다른 대화도 알림
$script:ConvFilterText       = ''
$script:ConvUnreadOnly       = $false
$script:UserFilterText       = ''
$script:ChatLoadTake         = 0      # 현재 대화 로드 목표 건수 (0=PageSize)
...
function Show-LoginDialogInputBox {
    <#
    .SYNOPSIS
        InputBox 3단계 로그인 (Form 즉시 닫힘 폴백).
    #>
    param(
...
function Show-LoginDialog {
    <#
    .SYNOPSIS
        서버/ID/PW 로그인. Form 이 즉시 닫히면 InputBox 폴백.
    #>
    param(
        [string]$DefaultUserId = '',
        [string]$DefaultApiBase = ''
    ...
    # 환경에 따라 WinForms 모달이 즉시 닫히는 경우가 있어
    # 우선 InputBox 를 기본으로 사용 (안정). Form 은 옵션.
    # 기본: InputBox (안정). 필요시 Form 시도 후 폴백도 유지.
...
function Invoke-LoadOlderChatMessages {
    <#
    .SYNOPSIS
        현재 대화에서 더 이전 메시지 로드 (pageSize 단위 증가).
    #>
    if (-not $script:CurrentChatMD5) { return }
    if ($script:isPolling -or $script:isSending -or $script:isSearching) {
        Set-StatusSafe '동기화/전송 중 - 잠시 후 다시'
        return
    }
    ...
    if ($script:ChatRichTextBox -and -not $script:ChatRichTextBox.IsDisposed) {
        Show-ChatMessages -RichTextBox $script:ChatRichTextBox -Messages $msgs
        try {
            $script:ChatRichTextBox.SelectionStart = 0
            $script:ChatRichTextBox.ScrollToCaret()
        } catch { }
    }
...
function New-MainForm {
    ...
    # TableLayout: 검색행 + 목록 (Dock 겹침으로 첫 줄 가려지는 문제 방지)
...
function Get-SessionCookieHeader {
    <#
    .SYNOPSIS
        현재 HttpSession 쿠키를 Cookie 헤더 문자열로 (백그라운드 HTTP용).
    #>
...
function New-ChatForm {
    ...
    [void]$searchTable.ColumnStyles.Add((New-Object System.Windows.Forms.ColumnStyle([System.Windows.Forms.SizeType]::Absolute, 56.0)))
    [void]$searchTable.ColumnStyles.Add((New-Object System.Windows.Forms.ColumnStyle([System.Windows.Forms.SizeType]::Absolute, 56.0)))
    [void]$searchTable.ColumnStyles.Add((New-Object System.Windows.Forms.ColumnStyle([System.Windows.Forms.SizeType]::Absolute, 56.0)))
    ...
    $btnOlder.Margin = New-Object System.Windows.Forms.Padding(2, 0, 2, 0)
    $btnOlder.Add_Click({ Invoke-LoadOlderChatMessages })
    $searchTable.Controls.Add($btnOlder, 2, 0)
    ...
    $searchTable.Controls.Add($btnRecent, 3, 0)
    ...
    $rtb.ReadOnly = $true
    $rtb.BackColor = [System.Drawing.Color]::FromArgb(236, 239, 241)
...
function Initialize-ChatContent {
    <#
    .SYNOPSIS
        md5 대화의 최근 메시지를 RichTextBox에 로드 (최대 ChatPageSize).
    .DESCRIPTION
        P2: 최신 월부터 필요 분량만 로드. 전체 이력 full load 하지 않음.
    #>
    param(
        [Parameter(Mandatory = $true)]
        [string]$Md5
...
function Complete-MessagePollFromRows {
    <#
    .SYNOPSIS
        폴링 결과 rows 처리 (로컬 저장/UI). UI 스레드 전용.
    #>
    param(
        [AllowEmptyCollection()]
        [array]$RawList = @(),
...
function Invoke-MessagePoll {
    <#
    .SYNOPSIS
        동기 폴링 (초기 동기화용). HTTP + 처리를 UI 스레드에서 수행.
    #>
...
function Complete-MessagePollAsync {
    <#
    .SYNOPSIS
        백그라운드 HTTP 완료 콜백 (UI 스레드, BeginInvoke).
    #>
    param($Result)
...
function Start-MessagePollAsync {
    <#
    .SYNOPSIS
        호환용. 안정성을 위해 동기 폴링으로 위임 (P4 비동기 비활성).
    #>
    Invoke-PollTimerTick
}


function Invoke-PollTimerTick {
    if ($script:IsExiting) { return }
    if ($script:isSending -or $script:isSearching) { return }
...
function Complete-ApplicationShutdown {
    ...
    # 백그라운드 HTTP 종료 대기 (최대 ~0.5s, UI 펌프 없음)
    try {
        $wait = 0
